...
impl Handler {
    ...
    #[instrument(skip(self, rec), fields(event_id = %rec.event.event_id))]
    pub async fn handle_event(&self, attempt: u32, rec: EventRecord, processing_time: Duration) {
        ...
        tracing::debug!(attempt = %attempt, "Handling event");
    ...
    #[instrument(skip(self, id), fields(event_id = %id))]
    pub async fn run(&mut self, id: String, processing_time: Duration) {
        ...
        match self.store.try_start_processing(&id).await {
            ...
            StartResult::NotFound => {
                ...
                tracing::error!("Event not found when trying to start processing");
            },
            ...
            StartResult::SkipAlreadyCompleted => {
                tracing::debug!("Event already completed, skipping");
            },
            StartResult::SkipNotRunnable => {
                tracing::debug!("Event not runnable (maybe being processed by another worker), skipping");
            },
        }
    }
}
